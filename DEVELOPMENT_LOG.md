...
- 需求来源：
  - 用户希望 macOS 版可以一次选择几个视频并一起生成高光片段。
- 本次修改：
  - 版本号更新为 `macOS v1.3.3`。
  - “选择视频”改为支持一次多选多个视频文件。
  - 扫描文件夹后，视频列表默认全选；点击“开始剪辑”会按队列依次处理选中的多个视频。
  - 用户也可以在视频列表里手动选择一个或多个视频，只处理当前选中的条目。
  - 批量剪辑时日志会显示总视频数、当前处理到第几个视频、单个视频导出数量和跳过错误。
  - 所有视频导出的高光片段会汇总显示在右侧 Highlights 卡片区，仍保留低清预览、高清播放、定位和删除。
- Windows 同步提示：
  - Windows 端可同步“列表多选 + 队列处理 + 汇总高光卡片”的行为。
...
- 需求来源：
  - 用户要求检查最新 Windows 开发日志，并把可复用更新同步到 macOS。
  - Windows 远端最新为 `865e364`，开发日志显示 v1.4.2 只新增参数提示、预计耗时和开始按钮运行态。
- 本次修改：
  - macOS 参数区标题同步为 `参数（建议不要动，除非你懂什么意思）`，英文同步为 `Settings (do not change unless you know what they mean)`。
  - 点击开始剪辑前，处理日志写入粗略预计耗时，估算依据包括视频时长、最多分析秒数、识别帧率和快速无损截取开关。
  - 开始剪辑后主按钮文案变为 `剪辑中`，任务结束后恢复 `开始剪辑`。
  - 中英文切换时，如果剪辑任务仍在运行，主按钮运行态文案会跟随语言刷新。
- Windows 同步提示：
  - 本次只同步 Windows v1.4.2 的产品行为，不迁移 Windows PySide6、Mica 或 Release 更新清单实现。

### 修复记录：更新误报、输入框居中和 App 图标

- 需求来源：
  - 用户在 PDF 标注中反馈：当前版本和最新版本一致时仍提示发现新版。
  - 输出目录输入框文字仍然偏上。
  - Dock/Finder 中还是默认应用图标，希望改成专属 App 图标。
- 本次修改：
  - 更新检查新增短 SHA 兜底比较；当前 SHA 和远端 SHA 前 7 位一致时视为已是最新，避免 `当前 a759a27，最新 a759a27` 仍弹新版本提示。
  - 单行输入框改用更稳的垂直居中绘制逻辑，并同步编辑/选择状态的文本 rect。
  - 按钮统一设置字体 weight，减少胶囊按钮内文字偏上的视觉问题。
  - 新增 Apple 黑灰玻璃风 `ValorantHighlightClipper.icns`，并接入 PyInstaller `BUNDLE icon` 配置，让 `.app`、Dock、Finder 显示专属图标。
- Windows 同步提示：
  - Windows 端可同步“短版本一致即最新”的更新比较逻辑。
  - 图标资源为 macOS `.icns`，Windows 端后续需要单独生成 `.ico`。
